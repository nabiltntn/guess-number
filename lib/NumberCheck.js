if (Meteor.isClient) {
  ...
  NumberGame = {
    checkNumber(number) {
      ...
      let digitNotToCheck = _.difference(_.map(_.range(1,11), (val) => {return val.toString()}), digitToCheck)
      ;
      ...
      let allCheck = _.reduce(propsToCheck,
        function(res, currentFunc){
          return res && currentFunc(numArray)
        }, true
      )
      
      let allNotCheck = _.reduce(propsNotToCheck,
        function(res, currentFunc){
          return res && currentFunc(numArray)
        }, false
      )
  ...
  NumberFunction = {
    ...
    '1' : function (number) {// chiffres suite décroissante
      let valid = false
      if(number.length > 1) {
      for (let i = 0, length = number.length; i < length - 1 ; i++) {
    ...
    '4' : function (number) {// chiffres tous différents
      return number.length >= 4 && number[3] % 2 == 0
    },
    '5' : function (number) {// chiffres tous différents
      let produit = 1
    ...
    '6' : function (number) {// chiffres tous différents
      let valid = false
      if(number.length >=3) {
